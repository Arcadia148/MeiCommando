...
const { ArgumentInfo } = require('./argument'); // eslint-disable-line no-unused-vars
...
class Command {
	...
	constructor(client, info) {
		...
		/**
		 * Whether the command can only be run in a specific guild
		 * @type {?string[]}
		 */
		this.guild = info.guild || null;
	 ...
	 * (built-in reasons are `guild`, `guildOnly`, `nsfw`, `permission`, `throttling`, and `clientPermissions`)
	...
	static validateInfo(client, info) { // eslint-disable-line complexity
		...
		if (info.guild && (!Array.isArray(info.guild) || info.guild.some(guild => typeof guild !== 'string'))) {
			throw new TypeError('Command guild must be an Array of strings.');
		}

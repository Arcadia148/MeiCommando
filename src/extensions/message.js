...
module.exports = Structures.extend('Message', Message => {
	...
	class CommandoMessage extends Message {
		...
		async run() { // eslint-disable-line complexity
			...
			if (this.command.guild) {
				if (typeof this.command.guild === 'string') {
					if (this.command.guild !== this.guild.id) {
						...
						return this.command.onBlock(this, 'guild');
					}
				} else if (this.command.guild instanceof Array) {
					if (!this.command.guild.includes(this.guild.id)) {
						this.client.emit('commandBlock', this, 'guild');
						return this.command.onBlock(this, 'guild');
					}
				} else { throw new RangeError(`Type guild Can Only be a type of String or Array | ${this.command.guild}`); }
			}
			...
			const { typingCount } = this.channel;
			try {

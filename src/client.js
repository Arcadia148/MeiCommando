...
class CommandoClient extends discord.Client {
	 ...
	 * @property {string|string[]|Set<string>} [contributor] - ID of the bot contributors's Discord user, or multiple IDs
	...
	constructor(options = {}) {
		...
		// Fetch the contributors(s)
		if (options.contributor) {
			this.once('ready', () => {
				if (options.contributor instanceof Array || options.contributor instanceof Set) {
					for (const contributor of options.contributor) {
						this.users.fetch(contributor).catch(err => {
							this.emit('warn', `Unable to fetch contributor ${contributor}.`);
							this.emit('error', err);
						});
					}
				} else {
					this.users.fetch(options.contributor).catch(err => {
						this.emit('warn', `Unable to fetch contributor ${options.contributor}.`);
						this.emit('error', err);
					});
				}
			});
	...
	/**
	 * contributors of the bot, set by the {@link CommandoClientOptions#contributor} option
	 * <info>If you simply need to check if a user is an contributor of the bot, please instead use
	 * {@link CommandoClient#isContibutor}.</info>
	 * @type {?Array<User>}
	 * @readonly
	 */
	get contributors() {
		if (!this.options.contributor) return null;
		if (typeof this.options.contributor === 'string') return [this.users.cache.get(this.options.contributor)];
		const contributors = [];
		for (const contributor of this.options.contributor) contributors.push(this.users.cache.get(contributor));
		return contributors;
	}
	 ...
	 */
	isContributor(user) {
		if (!this.options.contributor) return false;
		user = this.users.resolve(user);
		if (!user) throw new RangeError('Unable to resolve user.');
		if (typeof this.options.contributor === 'string') return user.id === this.options.contributor;
		if (this.options.contributor instanceof Array) return this.options.contributor.includes(user.id);
		if (this.options.constructor instanceof Set) return this.options.contributor.has(user.id);
		throw new RangeError('The client\'s "contributor" option is an unknown value.');
	}
